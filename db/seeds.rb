...
#PAGES
Page.find_or_create_by(url: "about") do |page|
  page.title = "О проекте"
  page.content = "<p>Четко сформулированная концепция — это основа основ создания и развития бизнеса в сфере индустрии красоты. Эстетика, специализация и атмосфера вашего салона могут быть любыми, но они должны быть. Причем такими, чтобы выгодно отличать его от всех прочих салонов. Чтобы ваши клиенты понимали и чувствовали, что он создан именно для них.</p>"
end

Page.find_or_create_by(url: "brand") do |page|
  page.title = "О бренде"
  page.content = "<p>Наши идеи помогают стать успешнее всем людям, которые их разделяют, а наши методы — сделать успешнее каждый бизнес, в котором они применяются.</p>"
end
